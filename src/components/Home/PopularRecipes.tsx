...
export function PopularRecipes() {
  ...
  const PopularRecipes = recipes.filter(
    (recipe: Recipe) => recipe.popular === 'true'
  )
              ...
              <div className="p-1 relative">
                <Card className="relative">
                  ...
                  <CardContent className="px-6">
                    ...
                    <CardDescription>
                      {recipe.description.length > descLength
                        ? `${recipe.description.slice(0, descLength)}...`
                        : recipe.description}
                    </CardDescription>
                    <Link
                      href={`/recipe/${recipe._id}`}
                      className="mt-3 bg-primary text-white font-bold dark:bg-white dark:text-black text-center py-2 rounded-md block"
                    >
                      Cook This
                    </Link>
                  </CardContent>
