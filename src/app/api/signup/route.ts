...
export async function POST(request: Request) {
  ...
  try {
    const { username, email, password, restaurant, gender, usertype } =
      await request.json()
    const lowerCaseUsername = username.toLowerCase()

    if (!usertype) {
      return NextResponse.json(
        createErrorResponse('Usertype is required', 400),
        { status: 400 }
      )
    }
    ...
    if (existingUserByUsernameVerified) {
      return NextResponse.json(
        createErrorResponse('Username already exists and is verified.', 400),
        { status: 400 }
      )
    }
    ...
    if (existingUserByEmail) {
      if (existingUserByEmail.isVerified) {
        return NextResponse.json(
          createErrorResponse('This Email Already Exists', 400),
          { status: 400 }
        )
      }
      ...
      if (!emailrespo.success) {
        return NextResponse.json(
          createErrorResponse('Error in Email send', 500),
          { status: 500 }
        )
      }
    ...
    return NextResponse.json(
      createSuccessResponse(null, 'Verification Code Sent to Your Email', 200),
      { status: 200 }
    )
  } catch (error: any) {
    console.error(error)
    return NextResponse.json(createErrorResponse('An Error Occurred', 500), {
      status: 500
    })
  }
}
